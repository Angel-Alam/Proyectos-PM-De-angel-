...
void Lectura1(int A[][12]) {
    ...
    while (MES != -1 && PRO != -1 && CAN != -1) {
        A[MES - 1][PRO - 1] += CAN; // Actualiza las ventas
...
void Funcion1(int A[][12], int F, int C, float V1[], float V2[]) {
    ...
    for (I = 0; I < F; I++) {
        SUM = 0;
        for (J = 0; J < C; J++)
            SUM += A[I][J]; // Suma las ventas del producto
        V2[I] = V1[I] * SUM; // Calcula el total de ventas
...
void Funcion2(float A[], int C) {
    ...
    float SUM = 0.0;
    for (I = 0; I < C; I++)
        SUM += A[I]; // Suma el total de ventas
...
void Funcion3(float A[], int C) {
    ...
    int I, TPR = 0;
    float VEN = A[0]; // Inicializa VEN con el total del primer producto
    
    for (I = 1; I < C; I++) {
        if (VEN < A[I]) { // Compara si el actual es mayor
            TPR = I;
            VEN = A[I];
        }
    }

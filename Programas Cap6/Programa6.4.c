...
const int MAX = 100; // Definición de la constante MAX
...
int main(void) {
    ...
    do {
        ...
        scanf("%d", &N);
    } while (N > MAX || N < 1); // Verifica que el tamaño del arreglo sea válido.
...
int Simetrico(int A[][MAX], int T) {
    /* La función Simetrico se utiliza para determinar si el arreglo bidimensional
    ...
    int I = 0, J, F = 1; // Inicializa el índice y la bandera de simetría
    while ((I < T) && F) {
        J = 0;
        while ((J < I) && F) {
            if (A[I][J] == A[J][I])
                ...
                F = 0; // Si hay una desigualdad, no es simétrico
        }
        I++;
    }
    return F; // Regresa el estado de simetría
}

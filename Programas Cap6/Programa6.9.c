...
int main(void) {
    ...
    do {
        ...
        scanf("%d", &TAM);
    } while (TAM > 10 || TAM < 1); // Se corrige la condición con "||"
...
float Mod0(int A[][10], int K, int M) {
    ...
    return (SUM != 0) ? (PRO / SUM) : 0; // Previene división por cero
}
...
float Mod1(int A[][10], int N) {
    ...
    int I;
    float SUM = 0.0;
    for (I = 0; I < N; I++) { // Cambiado <=N a <N para evitar acceso fuera de límites
        SUM += A[N][I];
    }
    return SUM;
}
...
void Imprime(float B[], int N) {
    ...
    int I;
    for (I = 0; I < N; I++) {
        printf("\nElemento %d: %.2f ", I + 1, B[I]); // Cambiado a I + 1 para mejor legibilidad
    }
}
